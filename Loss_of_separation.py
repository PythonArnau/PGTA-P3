...
def extract_callsign_airline(callsign):
    if not callsign:
        return None
    return callsign[:3].upper()


def format_loss_reason(loss_details):
    if not loss_details:
        return "NONE"
    normalized = loss_details.upper()
    if "HORIZONTAL" in normalized:
        return "HORIZONTAL"
    if "OPERATIONAL" in normalized:
        return "OPERATIONAL"
    if "LEADER OUT OF GEO FILTER" in normalized:
        return "LEADER OUT OF GEO FILTER"
    return normalized


def isSameSID(sidL, sidF):
    if sidL == sidF:
        return "YES"
    return "NO"


def export_combined_separation_excel(file, distances_pairs, radar_res, wake_res, loa_res):
    radar_map = {pair.pair: pair for pair in radar_res}
    wake_map = {pair.pair: pair for pair in wake_res}
    loa_map = {pair.pair: pair for pair in loa_res}

    rows = []
    for pair in distances_pairs:
        pair_key = pair.pair
        radar_pair = radar_map.get(pair_key)
        wake_pair = wake_map.get(pair_key)
        loa_pair = loa_map.get(pair_key)
        ...
        tma_loss = "NO"
        tma_reason = "NONE"
        if radar_pair and radar_pair.tma_details:
            tma_item = radar_pair.tma_details[0]
            tma_dist = tma_item.horizontal_separation_nm
            tma_alt = tma_item.altitude_separation_ft
            tma_time = tma_item.time
            tma_loss = radar_pair.tma_loss
            tma_reason = format_loss_reason(tma_item.loss_details)
        ...
        twr_alt = None
        twr_dist2 = None
        if radar_pair and radar_pair.twr_details:
            twr_item = radar_pair.twr_details[0]
            twr_dist = twr_item.horizontal_separation_nm
            twr_time = twr_item.time
            twr_alt = twr_item.altitude_separation_ft
            twr_dist2 = twr_item.distance_twr_nm
            twr_loss = radar_pair.twr_loss
            twr_reason = format_loss_reason(twr_item.loss_details)

        wake_tma_loss = "NO"
        wake_tma_reason = "NONE"
        wake_twr_loss = "NO"
        wake_twr_reason = "NONE"
        if wake_pair:
            if wake_pair.tma_details:
                wake_tma_item = wake_pair.tma_details[0]
                wake_tma_loss = wake_pair.tma_loss
                wake_tma_reason = format_loss_reason(wake_tma_item.loss_details)
            if wake_pair.twr_details:
                wake_twr_item = wake_pair.twr_details[0]
                wake_twr_loss = wake_pair.twr_loss
                wake_twr_reason = format_loss_reason(wake_twr_item.loss_details)

        loa_twr_loss = "NO"
        loa_twr_reason = "NONE"
        loa_lead_class = None
        loa_foll_class = None
        if loa_pair and loa_pair.twr_details:
            loa_twr_item = loa_pair.twr_details[0]
            loa_twr_loss = loa_pair.twr_loss
            loa_twr_reason = format_loss_reason(loa_twr_item.loss_details)
            loa_lead_class = getattr(loa_pair, 'lead_class', None)
            loa_foll_class = getattr(loa_pair, 'foll_class', None)

        wake_min_hor_sep = wake_pair.min_hor_sep if wake_pair is not None and getattr(wake_pair, 'min_hor_sep', None) is not None else "COMBINATION DONT APPLY"
        loa_min_hor_sep = loa_pair.min_hor_sep if loa_pair is not None and getattr(loa_pair, 'min_hor_sep', None) is not None else "COMBINATION DONT APPLY"

        sameSID = isSameSID(pair.sid_lead_group, pair.sid_foll_group)

        row = {
            'Leader': pair.leader,
            'Follower': pair.follower,
            'Sid_lead': pair.sid_lead,
            'Sid_foll': pair.sid_foll,
            'Sid_lead_G': pair.sid_lead_group,
            'Sid_foll_G': pair.sid_foll_group,
            'SameSID': sameSID,
            'Leader_AC': pair.AC_lead,
            'Follower_AC': pair.AC_foll,
            'Leader_Airline': extract_callsign_airline(pair.leader),
            'Follower_Airline': extract_callsign_airline(pair.follower),
            'Wake_lead': pair.wake_lead,
            'Wake_foll': pair.wake_foll,
            'LOA_lead_class': loa_lead_class,
            'LOA_foll_class': loa_foll_class,
            'Wake_min_hor_sep': wake_min_hor_sep,
            'LOA_min_hor_sep': loa_min_hor_sep,
            'TMA_dist': tma_dist,
            'TMA_alt': tma_alt,
            'TMA_time': tma_time,
            'Radar_TMA_loss': tma_loss,
            'Radar_TMA_reason': tma_reason,
            'Wake_TMA_loss': wake_tma_loss,
            'Wake_TMA_reason': wake_tma_reason,
            'TWR_dist': twr_dist,
            'Dist2TWR': twr_dist2,
            'TWR_alt': twr_alt,
            'TWR_time': twr_time,
            'Radar_TWR_loss': twr_loss,
            'Radar_TWR_reason': twr_reason,
            'Wake_TWR_loss': wake_twr_loss,
            'Wake_TWR_reason': wake_twr_reason,
            'LOA_TWR_loss': loa_twr_loss,
            'LOA_TWR_reason': loa_twr_reason,
        }
