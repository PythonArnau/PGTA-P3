...
def detect_turn(flights, hdg_threshold, ra_threshold):
    ...
    for flight in departure_flights:
        ...
        turn.performance_class = flight.performance_class

        radial_cross = crosses_radial(flight)
        turn.cross_radial = radial_cross

        valid_records = [rec for rec in flight.records if rec.h_ft is not None and min_alt <= rec.h_ft <= max_alt]
...
def interpolate_RA(records, time):
    ...
    interpolated_TTA = prev.TTA + alpha * (next.TTA - prev.TTA)
    ...
    for t in turn_list:
        if t.turn_start:
            ...
            data.append({
                ...
                "SID": t.SID,
                "Wake": t.wake,
                "Performance class": t.performance_class
            })
        else:
